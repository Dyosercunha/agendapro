(() => {
  ...
  const APP_BASE_URL = 'https://calendarproapp.vercel.app';
    ...
    root.innerHTML = `
      <main class="platformApp">
        ...
        <section class="platformStats">
          ${statCard('Barbearias', data.stats?.total || 0)}
          ${statCard('Ativas', data.stats?.active || 0)}
          ${statCard('Bloqueadas', data.stats?.blocked || 0)}
          ${statCard('Próximo vencimento', data.stats?.next_billing || '—')}
        </section>
        ...
        <section class="platformGrid">
          ...
          <div class="platformCard">
            ...
            <div class="platformShopList">
              ${shops.length ? shops.map(shopRow).join('') : '<p class="platformMuted">Nenhuma barbearia cadastrada.</p>'}
            </div>
          </div>
        </section>

        <section class="platformCard" id="platformEditor">
          ${selectedShop ? editShopForm(selectedShop) : '<p class="platformMuted">Selecione uma barbearia para editar plano, status e funções.</p>'}
        </section>
      </main>
    `;
  ...
  function newShopForm() {
    return `
      <form class="platformForm" data-new-shop>
        ...
        <input name="name" placeholder="Barbearia do João" required />
        <label>Slug do link</label>
        <input name="slug" placeholder="barbearia-do-joao" />
        <label>WhatsApp</label>
        <input name="whatsapp" placeholder="5551999999999" />
        <label>E-mail do dono</label>
        <input name="owner_email" type="email" placeholder="dono@email.com" required />
        <div class="platformTwoCols">
          <span>
            <label>Plano</label>
            <select name="plan">
        ...
        <input name="pix_key" placeholder="CPF, CNPJ, e-mail ou telefone" />
        <label>Cor principal</label>
        <input name="theme_color" type="color" value="#22c55e" />
  ...
  function shopRow(shop) {
    return `
      <article class="platformShop">
        <div>
          <strong>${shop.name || 'Sem nome'}</strong>
          <span>${shop.slug}</span>
          <small>${shop.owner_email || 'Sem e-mail'} · ${shop.plan || 'sem plano'}</small>
        </div>
        <div class="platformShopActions">
          ${shopStatus(shop.monthly_status)}
          <button type="button" data-edit-shop="${shop.slug}">Editar</button>
  ...
  function editShopForm(shop) {
    ...
    return `
      <div class="platformTitle">
        ...
        <button type="button" class="platformSecondary" data-close-editor>Fechar</button>
      </div>
      <form class="platformForm" data-edit-shop-form>
        ...
        <label>WhatsApp</label>
        <input name="whatsapp" value="${shop.whatsapp || ''}" />
        <label>E-mail do dono</label>
        <input name="owner_email" type="email" value="${shop.owner_email || ''}" />
        <div class="platformTwoCols">
          <span>
            <label>Plano</label>
            <select name="plan">
              ${['starter','professional','premium'].map(plan => `<option value="${plan}" ${shop.plan === plan ? 'selected' : ''}>${plan}</option>`).join('')}
            </select>
          </span>
          <span>
            <label>Status</label>
            <select name="monthly_status">
              ${['active','trial','blocked'].map(status => `<option value="${status}" ${shop.monthly_status === status ? 'selected' : ''}>${status}</option>`).join('')}
            </select>
          </span>
        </div>
        <label>Vencimento</label>
        <input name="next_billing_date" type="date" value="${shop.next_billing_date || ''}" />
        <label>Endereço</label>
        <input name="address" value="${shop.address || ''}" />
        <label>Chave PIX</label>
        <input name="pix_key" value="${shop.pix_key || ''}" />
        <label>Cor principal</label>
        <input name="theme_color" type="color" value="${shop.theme_color || '#22c55e'}" />
      ...
      <div class="platformFeatures">
        ...
        ${featureKeys.map(key => {
          ...
          return `
            <label class="platformFeature">
              <span>
                <strong>${featureLabels[key]}</strong>
                <small>${key}</small>
              </span>
              <span class="featureChecks">
                <em>Liberado</em><input type="checkbox" data-feature-released="${key}" ${item.released ? 'checked' : ''} />
                <em>Ativo</em><input type="checkbox" data-feature-enabled="${key}" ${item.enabled ? 'checked' : ''} />
              </span>
            </label>
          `;
        }).join('')}
        <button type="button" class="platformPrimary" data-save-features="${shop.slug}">Salvar funções</button>
      </div>
    `;
  }
  ...
  async function createShop(form) {
    ...
    const { data, error } = await sb.rpc('create_barbershop_full', {
      name_input: name,
      slug_input: slug,
      whatsapp_input: moneyText(formValue(form, 'whatsapp')),
  ...
  async function updateShop(form) {
    const sb = await getSupabase();
    const { error } = await sb.rpc('update_platform_barbershop', {
  ...
  async function saveFeatures(slug) {
    ...
    const features = Object.keys(featureLabels).map(key => ({
      feature_key: key,
      released: !!document.querySelector(`[data-feature-released="${key}"]`)?.checked,
      enabled: !!document.querySelector(`[data-feature-enabled="${key}"]`)?.checked,
    }));
    const { error } = await sb.rpc('save_platform_feature_flags', {
      target_slug: slug,
  ...
  function bindDashboardEvents() {
    ...
    newForm?.querySelector('[name="name"]')?.addEventListener('input', event => {
      ...
      if (slugInput && !slugInput.dataset.manual) slugInput.value = makeSlug(event.target.value);
    });
    newForm?.querySelector('[name="slug"]')?.addEventListener('input', event => {
      event.target.dataset.manual = 'true';
      event.target.value = makeSlug(event.target.value);
    });
    ...
    document.querySelectorAll('[data-edit-shop]').forEach(button => {
      button.addEventListener('click', () => {
        selectedShop = (dashboardData.barbershops || []).find(shop => shop.slug === button.dataset.editShop);
        renderDashboard();
        document.getElementById('platformEditor')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      });
    });

    document.querySelector('[data-close-editor]')?.addEventListener('click', () => {
      selectedShop = null;
      renderDashboard();
    });
    ...
    document.querySelector('[data-save-features]')?.addEventListener('click', async event => {
